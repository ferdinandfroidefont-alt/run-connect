...
- **`runconnect-loading-splash.gif`** (optionnel) : GIF **authentique** (GIF89a) pour l’animation plein écran. `LoadingScreen` l’essaie en premier (`RUCONNECT_LOADING_SCREEN_GIF_URL`).
- **`runconnect-loading-splash.jpg`** : image fixe de secours (export Figma / fichier chat souvent en JPEG). Utilisée si le `.gif` est absent ou invalide (`RUCONNECT_LOADING_SCREEN_FALLBACK_URL`).
