-- Nouvelle fonction pour les suggestions d'amis avec ordre de priorité défini
...
AS $$
  WITH user_follows_status AS (
    -- Utilisateurs déjà suivis ou avec demandes en cours
    SELECT 
      CASE 
  ...
  mutual_friends_data AS (
    ...
    WHERE p.user_id != current_user_id
      AND p.user_id NOT IN (SELECT user_id FROM user_follows_status WHERE user_id IS NOT NULL)
  ...
  club_members_data AS (
    ...
    WHERE gm.conversation_id IN (SELECT conversation_id FROM user_groups)
      ...
      AND gm.user_id NOT IN (SELECT user_id FROM user_follows_status WHERE user_id IS NOT NULL)
    GROUP BY gm.user_id
  ),
  friends_of_friends AS (
    ...
    CROSS JOIN LATERAL (
      ...
      FROM user_follows uf1  -- Mes amis
      JOIN user_follows uf2 ON uf1.following_id = uf2.follower_id  -- Amis de mes amis
      JOIN user_follows uf3 ON uf2.following_id = uf3.follower_id  -- Amis d'amis d'amis
      WHERE uf1.follower_id = current_user_id
        AND uf1.status = 'accepted'
        AND uf2.status = 'accepted'
