...
export default function MySessions() {
  ...
  const loadUserSessions = async () => {
    ...
    try {
      ...
      let query = supabase.from('sessions').select('*').eq('organizer_id', user.id).order('scheduled_at', {
        ascending: false
      });
      const {
        data,
  ...
  const loadSessionParticipants = async (sessionId: string) => {
    try {
      ...
      } = await supabase.from('session_participants').select('*').eq('session_id', sessionId);
      if (participantsError) throw participantsError;
      const participantsWithProfiles = [];
      for (const participant of participantsData || []) {
        const {
          data: profile
        } = await supabase.from('profiles').select('username, display_name, avatar_url').eq('user_id', participant.user_id).single();
  ...
  const loadUserRoutes = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('routes').select('*').eq('created_by', user.id).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      setRoutes(data || []);
  ...
  const handleSessionUpdated = async () => {
    ...
    if (selectedSession) {
      const {
        data
      } = await supabase.from('sessions').select('*').eq('id', selectedSession.id).single();
      if (data) {
  ...
  const handleDeleteSession = async () => {
    ...
    try {
      ...
      const {
        error: participantsError
      } = await supabase.from('session_participants').delete().eq('session_id', selectedSession.id);
      if (participantsError) throw participantsError;

      // Supprimer les demandes de participation
      const {
        error: requestsError
      } = await supabase.from('session_requests').delete().eq('session_id', selectedSession.id);
      if (requestsError) throw requestsError;

      // Supprimer la séance
      const {
        error
      } = await supabase.from('sessions').delete().eq('id', selectedSession.id).eq('organizer_id', user?.id);
      if (error) throw error;
  ...
  const getStatusBadge = (session: UserSession) => {
    ...
    return isUpcoming ? <Badge variant="default">À venir</Badge> : <Badge variant="secondary">Terminée</Badge>;
  };
  if (selectedSession) {
    return <>
      <div className="container mx-auto px-4 py-6 pb-24">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {new Date(selectedSession.scheduled_at) >= new Date() && <Button variant="outline" size="sm" onClick={handleEditClick} className="flex items-center gap-2">
                <Edit className="h-4 w-4" />
                Modifier
              </Button>}
            <Button variant="destructive" size="sm" onClick={handleDeleteSession} className="flex items-center gap-2">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start gap-4">
              {selectedSession.image_url && <img src={selectedSession.image_url} alt={selectedSession.title} className="w-24 h-24 object-cover rounded-lg flex-shrink-0" />}
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mt-2">
                  <div className="flex items-center gap-1">
                    ...
                    {format(new Date(selectedSession.scheduled_at), 'PPP à HH:mm', {
                      locale: fr
                    })}
                  </div>
                  <div className="flex items-center gap-1">
          ...
          <CardContent>
            ...
            <div className="space-y-3">
              ...
              {participants.length > 0 ? <ScrollArea className="max-h-64">
                  <div className="space-y-2 pr-4">
                    {participants.map(participant => <div key={participant.id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/50 cursor-pointer hover:bg-muted transition-colors" onClick={() => navigateToProfile(participant.user_id)}>
                        <Avatar className="w-8 h-8">
                          <AvatarImage src={participant.profiles.avatar_url || undefined} alt={participant.profiles.username || participant.profiles.display_name} />
                          <AvatarFallback>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Inscrit le {format(new Date(participant.joined_at), 'dd/MM/yyyy à HH:mm', {
                          locale: fr
                        })}
                          </div>
                        </div>
                ...
                </ScrollArea> : <p className="text-sm text-muted-foreground">Aucun participant inscrit pour le moment.</p>}
            </div>
          </CardContent>
        </Card>
        
        <EditSessionDialog isOpen={isEditSessionDialogOpen} onClose={() => setIsEditSessionDialogOpen(false)} onSessionUpdated={handleSessionUpdated} session={selectedSession} />
        
        <ProfilePreviewDialog userId={showProfilePreview ? selectedUserId : null} onClose={closeProfilePreview} />
      </div>
      </>;
  }
  return <>
        ...
        <div className="fixed top-6 left-0 right-0 flex-shrink-0 z-50 space-y-4 pb-4 border-b border-border bg-[#0d0d33]">
          ...
          <div className="flex items-center justify-center mt-3">
            <div className="flex gap-2">
              ...
              <Button onClick={() => setCurrentView('routes')} variant={currentView === 'routes' ? 'default' : 'outline'} size="sm" className="gap-2">
                <Route className="h-4 w-4" />
                Mes Itinéraires
              </Button>
            </div>
      ...
      <div className="flex-1 overflow-y-auto pt-32" style={{
        ...
        loading ? <div className="text-center py-8">
            ...
            </div> : filteredSessions.length > 0 ? <div className="space-y-2">
              {filteredSessions.slice(0, 6).map(session => <Card key={session.id} className="cursor-pointer hover:shadow-sm transition-shadow" onClick={() => handleSessionClick(session)}>
                  <CardContent className="p-3 bg-[#10c8d5]">
                    <div className="flex items-center justify-between">
                      <div className="flex gap-2 flex-1 min-w-0">
                        {session.image_url && <img src={session.image_url} alt={session.title} className="w-10 h-10 object-cover rounded flex-shrink-0" />}
              ...
              {filteredSessions.length > 6 && <div className="text-center py-2">
              ...
              <p className="text-xs text-muted-foreground mb-3">
                {filter === 'all' ? "Créez votre première séance !" : filter === 'upcoming' ? "Planifiez votre prochaine activité !" : "Aucune séance terminée."}
              </p>
              <Button size="sm">Créer une séance</Button>
            </div> :
        // Routes View
        routesLoading ? <div className="text-center py-8">
              <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary mx-auto"></div>
              <p className="text-xs text-muted-foreground mt-2">Chargement des itinéraires...</p>
            </div> : routes.length > 0 ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {routes.map(route => <RouteCard key={route.id} route={route} onEdit={editRoute} onDelete={deleteRoute} />)}
      ...
      {/* Route Edit Dialog */}
      <RouteEditDialog isOpen={isRouteEditDialogOpen} onClose={() => {
        setIsRouteEditDialogOpen(false);
        setEditingRoute(null);
      }} route={editingRoute} onRouteUpdated={loadUserRoutes} />
    </div>
    </>;
}